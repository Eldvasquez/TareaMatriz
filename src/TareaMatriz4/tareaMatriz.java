...
public class tareaMatriz {
	...
	public static String Matriz[][]= new String [10][8];
	public static float Departamento[]= new float [5];
	public static Scanner entrada= new Scanner (System.in);
	public static DecimalFormat dc= new DecimalFormat("#.00");
	public static String deducciones,percepciones,departamento,sueldoBase;
	public static int opcion, ded,perc,dep=0,sB,igss;
	public static float isr=0,sueldoLiquido=0,total,floatIgss;
	...
	public static void DatosMatriz() {
		for(int i=0;i<10;i++) {
			...
			//
			floatIgss=(float) ((sB+ perc )*0.0483);
			Matriz[i][4]=String.valueOf(dc.format(floatIgss));
			//
			if(sB>=2500 && sB<=5000) {
				isr=(float) ((sB) * 0.03);
				Matriz[i][5]=String.valueOf(dc.format(isr));
			...
			Matriz[i][6]=String.valueOf(dc.format(sueldoLiquido));
			//
			Matriz[i][7]=Integer.toString(NoDepartamento(dep));
	...
	public static void MostrarDepartamento() {
	      for(int i = 0; i<10;i++)
	      { 
	    	  total = Float.parseFloat(Matriz[i][6]);
	      if(Matriz[i][7].equals("1"))
	      {
	    	  Departamento[0] = Departamento[0] + total;
	      }
	      if(Matriz[i][7].equals("2"))
	      {
	    	  Departamento[1] = Departamento[1] + total;
	      }
	       if(Matriz[i][7].equals("3"))
	      {
	    	   Departamento[2] = Departamento[2] + total;
	      }
	      if(Matriz[i][7].equals("4"))
	      {
	    	  Departamento[3] = Departamento[3] + total;
	      }
	      if(Matriz[i][7].equals("5"))
	      {
	...
	public static void MostrarMatriz() {
		System.out.println(" -------------------------------------------------------------------------------------------------------------------------------");
		System.out.println("|Nombres------Sueldo Base-----Deducciones-----Percepciones-----IGSS---------ISR---------Sueldo Liquido------No Departamento|\n");
		
		for(int j=0;j<10;j++) {
			System.out.println("|"+Matriz[j][0]+"\t\t"+Matriz[j][1]+"\t\t"+Matriz[j][2]+"\t\t"+Matriz[j][3]+"\t\t"+Matriz[j][4]+"\t\t"+Matriz[j][5]+"\t\t"+Matriz[j][6]+"\t\t\t"+Matriz[j][7]+"    |");
		System.out.println(" -------------------------------------------------------------------------------------------------------------------------------");
	}
	}
